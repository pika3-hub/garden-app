...
- **品種管理**: 作物に紐づく品種（アイコ、桃太郎など）を登録・管理
  ...
  - 植え付け時は作物または品種のいずれかを選択（統合モーダル）。品種の親作物を変更すると植え付けの作物表示も自動追従する（`plantings` は `crop_id` と `variety_id` の排他構造）
...
- **管理日記**: 栽培日記の記録・管理
  - 日付、天気、タイトル、本文の記録
  - 作物・場所・植え付け・収穫との関連付け（カード型モーダルで複数選択）
  - キーワード・日付範囲での検索機能
  - ダッシュボードに最新の日記を表示
...
- **植え付け一覧**: 現在の植え付け状況をステータス別（栽培中/栽培終了/すべて）に一覧表示
  - 作物名、品種（任意）、場所、植え付け日、最新観察記録の画像と日付を表示
...
- **収穫記録機能**: 収穫イベントの詳細な記録・管理
  ...
  - 場所詳細ページに収穫履歴を表示
  - 収穫記録一覧での種類バッジフィルター
  - 収穫一覧・植え付け詳細の両方から「収穫を記録」可能（新規フォームは植え付けをモーダルから選択、種類・場所バッジフィルタ付き）
  - 日記との関連付けが可能
...
- **タスク管理機能**: 栽培作業のタスクを管理
  ...
  - ステータス管理（未着手/進行中/完了）
  - 期限日の設定とカレンダー連携
  - 作物・場所・栽培記録との関連付け（カード型モーダルで複数選択）
  - ダッシュボードに未完了タスクを表示
- **詳細画面ナビゲーション**: 全詳細画面で前後データへの移動ボタンを表示
  - 一覧に戻らずに前後のデータへ移動可能
  - 各一覧画面の表示順に基づく前後移動
- **補足情報**: 作物・品種・場所・日記・タスク・収穫の詳細画面に外部情報を複数添付
  - 補足テキスト（メモ・注意事項など）
  - 追加画像（サムネイル・lightbox対応）
  - 外部サイトへのURL（リンククリックで遷移）
  - YouTube動画の埋め込み（詳細画面上で再生可能、タイムスタンプ対応）
- **一覧画面のグルーピング表示**: 主要な一覧をセクション見出し付きでグループ化
  - 日記・植え付け・収穫: 年月（`YYYY年M月`）ごとにグループ化
  - 作物: 作物種類ごと（見出しに作物アイコン表示）、件数多い順、1件のみは末尾「その他」にまとめる
  - 場所: 場所種類ごと、件数多い順、1件のみは末尾「その他」にまとめる
  - タスク: ステータス（進行中/未着手/完了）ごとにグループ化
  - フィルターバッジ操作で該当カードが0件になったグループは見出しごと自動非表示
...
│   │   ├── location.py     # 場所モデル
│   │   ├── planting.py      # 植え付けモデル（plantings テーブル、crop_id と variety_id は排他）
│   │   ├── diary.py        # 日記モデル
│   │   ├── harvest.py      # 収穫記録モデル
