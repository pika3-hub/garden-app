...
class Cooking:
    ...
    @staticmethod
    def get_crop_icons_batch(cooking_ids):
        """料理IDリストに対して関連作物アイコンを一括取得（effective_crop_idで重複排除）"""
        if not cooking_ids:
            return {}
        db = get_db()
        ...
        rows = db.execute(f'''
            SELECT cooking_id, effective_crop_id, icon_path, image_color, rel_id
            FROM (
                SELECT cr.cooking_id, c.id AS effective_crop_id,
                ...
                UNION ALL
                SELECT cr.cooking_id, cv.effective_crop_id,
        ...
        for row in rows:
            ...
            if cid not in result:
                result[cid] = {'seen': set(), 'icons': []}
            if ecid not in result[cid]['seen'] and row['icon_path']:
                result[cid]['seen'].add(ecid)
                result[cid]['icons'].append({
