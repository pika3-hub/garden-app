...
def list():
    ...
    # 親作物名でグループ化
    def _crop_key(v):
        return v.get('crop_name') or ''

    sorted_varieties = sorted(varieties, key=_crop_key)
    grouped = [(k, [item for item in g]) for k, g in groupby(sorted_varieties, key=_crop_key)]
