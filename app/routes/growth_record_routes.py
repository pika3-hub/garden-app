...
def index():
    ...
    if status == 'all':
        crops = LocationCrop.get_all_with_stats(status=None)
    else:
        crops = LocationCrop.get_all_with_stats(status=status)
...
def detail(location_crop_id):
    ...
    records = GrowthRecord.get_by_location_crop(location_crop_id)
...
def record_detail(record_id):
    """栽培記録個別詳細"""
    record = GrowthRecord.get_by_id(record_id)
    if not record:
...
def edit(record_id):
    ...
    location_crop = LocationCrop.get_by_id(record['location_crop_id'])
...
def update(record_id):
    ...
    try:
        GrowthRecord.update(record_id, data)
